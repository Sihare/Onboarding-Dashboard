<template>
  <div class="page">
    ...
    <div class="toolbar">
      <div class="search-wrap">
        <input
          v-model="search"
          class="search-input"
          placeholder="Search by order ID, participant or product…"
        />
    ...
    <p class="count">
      All orders (<span>{{ filtered.length }}</span
      >)
    </p>
    ...
    <table class="data-table" v-if="filtered.length">
      ...
      <tbody>
        <tr v-for="o in filtered" :key="o.id">
          <td style="font-family: monospace; font-weight: 600; color: #1a1c2e">
            {{ o.id }}
          </td>
    ...
    <div class="empty-state" v-else>
      ...
      <p class="empty-sub">
        {{
          search ? "Try a different search" : "Click New Order to create one"
        }}
      </p>
    </div>
      ...
      <div v-if="dlg.open" class="overlay" @click.self="closeDlg">
        <div class="modal modal-lg">
          <div class="modal-head">
            <i
              class="mdi mdi-clipboard-plus-outline"
            ...
            >
              <div class="field" style="grid-column: 1/-1">
                <label class="field-lbl"
                  >Participant Name <span style="color: #e91e63">*</span></label
                >
                <input
                  v-model="dlg.participant"
                  class="field-ctrl"
              ...
              <div class="field">
                <label class="field-lbl"
                  >Product <span style="color: #e91e63">*</span></label
                >
          ...
          <div class="modal-foot">
            ...
            <button class="btn-primary" @click="saveOrder">
              {{ dlg.editId ? "Save Changes" : "Create Order" }}
            </button>
          </div>
        </div>
      </div>
        ...
        <div class="modal modal-sm">
          ...
          <div class="modal-body">
            <p style="font-size: 14px; color: #666; line-height: 1.6">
              Delete order
              <strong style="color: #1a1c2e">{{ del.id }}</strong> for
              <strong style="color: #1a1c2e">{{ del.participant }}</strong
...
function saveOrder() {
  dlg.error = "";
  if (!dlg.participant.trim()) {
    dlg.error = "Participant name is required.";
    return;
  }
  if (dlg.qty < 1) {
    dlg.error = "Quantity must be at least 1.";
    return;
  }
  ...
  if (dlg.editId) {
    ...
    if (o)
      Object.assign(o, {
        participant: dlg.participant.trim(),
        product: dlg.product,
        qty: dlg.qty,
        status: dlg.status,
        date: dateStr,
...
function askDelete(o) {
  Object.assign(del, { open: true, id: o.id, participant: o.participant });
}
function doDelete() {
  orders.value = orders.value.filter((o) => o.id !== del.id);
  del.open = false;
}
</script>
